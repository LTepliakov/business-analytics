...
select 		COALESCE (ca.rep_month, gl.first_date) as report_month
			...
			{# , case 	when ca.bundle_type = 'mb' and ca.Master_Account_Name='Mobile Telecommunications Company - Zain Saudi Arabia' and ca.date_nk<'2023-07-01'
					...
					when ca.bundle_type = 'mb' and ca.Master_Account_Name='Mobile Telecommunications Company - Zain Saudi Arabia' and ca.date_nk>='2023-07-01'
							then 0.01*ca.units
					when ca.bundle_type = 'mb' then ca.charge
					when ca.current_selling_rate_value = 0 or ca.current_selling_rate_value = 1
							then ca.charge*COALESCE(sr.manual_sell_rate, sr.analytics_selling_rate, srp.parent_selling_rate)
					else 	ca.charge*ca.current_selling_rate_value
			  end as final_revenue_trf #}
			...
			, sa.Account_Type
from		{{ ref('oy_dbt_consumption_tariff_sr')}} as ca 
-------
left join 	(
				select 		* 
...
select 		--distinct
			...
			--, round(final_revenue_trf,2) as final_revenue_trf
...
on 			tm.odoo_id = base.Master_Account_ID_coalesce
